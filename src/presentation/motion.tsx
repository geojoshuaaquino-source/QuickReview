...
export function FadeIn({children,delay=0,style}:MotionProps){
 ...
 useEffect(()=>{if(reduced){opacity.setValue(1);translate.setValue(0);return}Animated.parallel([
  Animated.timing(opacity,{toValue:1,duration:T.motion.fast,delay,easing:Easing.out(Easing.cubic),useNativeDriver:true}),
  Animated.timing(translate,{toValue:0,duration:T.motion.enter,delay,easing:Easing.out(Easing.cubic),useNativeDriver:true}),
...
export function LiftIn({children,delay=0,style}:MotionProps){
 ...
 useEffect(()=>{if(reduced){opacity.setValue(1);scale.setValue(1);return}Animated.parallel([
  Animated.timing(opacity,{toValue:1,duration:T.motion.fast,delay,useNativeDriver:true}),
...
export function ScalePress({children,style,...props}:ScalePressProps){
 ...
 return <Pressable {...props} onPressIn={e=>{animate(T.motion.pressScale);props.onPressIn?.(e)}} onPressOut={e=>{animate(1);props.onPressOut?.(e)}}><Animated.View style={[style,{transform:[{scale}]}]}>{children}</Animated.View></Pressable>
}
...
export function Reveal({visible,children,style}:{visible:boolean;children:React.ReactNode;style?:StyleProp<ViewStyle>}){
 ...
 useEffect(()=>{if(reduced){opacity.setValue(visible?1:0);y.setValue(0);return}Animated.parallel([
  Animated.timing(opacity,{toValue:visible?1:0,duration:T.motion.fast,easing:Easing.out(Easing.cubic),useNativeDriver:true}),
  Animated.timing(y,{toValue:visible?0:8,duration:T.motion.standard,easing:Easing.out(Easing.cubic),useNativeDriver:true}),
