...
type Props = { decks: Deck[]; reviews: ReviewRecord[]; onStudy: (deckId: string) => void; onDecks: () => void; onLibrary: () => void };

export function HomeScreen({ decks, reviews, onStudy, onDecks, onLibrary }: Props) {
  ...
  return <ScrollView contentContainerStyle={s.scroll} showsVerticalScrollIndicator={false}>
    ...
    <View style={s.hero}>
      <Text style={s.eyebrow}>READY WHEN YOU ARE</Text><Text style={s.heroNumber}>{activeCards}</Text><Text style={s.heroLabel}>cards ready to study</Text>
      <Pressable disabled={!decks.length} onPress={() => onStudy(decks[0].id)} style={s.heroButton}><Text style={s.heroButtonText}>Start studying</Text><Text style={s.heroArrow}>→</Text></Pressable>
      <Text style={s.heroHint}>No schedule required · study at your pace</Text>
    </View>
    ...
    <View style={s.stats}>
      <Stat value={String(todayReviews.length)} label="Today" />
      <Stat value={`${successRate}%`} label="Good / Easy" />
      <Stat value={avgSeconds ? `${avgSeconds}s` : '—'} label="Avg. time" />
    </View>
    ...
    <Text style={s.sectionTitle}>Make something</Text><View style={s.actions}>
      <Pressable style={s.action} onPress={onDecks}><Text style={s.actionIcon}>＋</Text><Text style={s.actionTitle}>New deck</Text><Text style={s.actionHint}>Start with your own cards</Text></Pressable>
      <Pressable style={s.action} onPress={onLibrary}><Text style={[s.actionIcon,{backgroundColor:T.colors.greenSoft}]}>↑</Text><Text style={s.actionTitle}>Import</Text><Text style={s.actionHint}>Bring in notes or a file</Text></Pressable>
    </View>
  </ScrollView>;
}
