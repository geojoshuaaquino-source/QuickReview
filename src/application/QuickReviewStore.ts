...
type ImportItem={front:string;back:string;type?:CardType;tags?:string[]};
export function useQuickReview(repo:QuickReviewRepository){
 ...
 const importCards=useCallback(async(deckId:string,items:ImportItem[])=>{for(const item of items){try{const card=await service.createCard(deckId,item.front,item.back,item.type??'basic');if(item.tags?.length)await repo.updateCard({...card,tags:item.tags,updatedAt:now()})}catch(error){if(!(error instanceof Error)||!error.message.includes('same front and back'))throw error}}await refresh()},[refresh,repo,service]);
