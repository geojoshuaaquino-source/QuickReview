...
export function useQuickReview(repo: QuickReviewRepository) {
  ...
  const refresh = useCallback(async () => {
    const [nextDecks, nextReviews, nextSettings] = await Promise.all([service.listDecks(), repo.getReviews(), repo.getSettings()]);
    setDecks(nextDecks); setReviews(nextReviews); setSettings({...DEFAULT_SETTINGS,...nextSettings}); setLoaded(true);
  }, [repo, service]);
  useEffect(() => { refresh().catch(() => setLoaded(true)); }, [refresh]);

  const saveSettings = useCallback(async (next:AppSettings) => { const safe={...DEFAULT_SETTINGS,...next}; await repo.saveSettings(safe); setSettings(safe); }, [repo]);
  ...
  const createCard = useCallback(async (deckId:string,front:string,back:string,type:CardType='basic',tags:string[]=[]) => {
    const existing=decks.find(d=>d.id===deckId)?.cards??[]; const normalizedFront=front.trim().toLowerCase(); const normalizedBack=back.trim().toLowerCase();
    ...
    const card=await service.createCard(deckId,front,back,type); if(tags.length) await repo.updateCard({...card,tags,updatedAt:now()}); await refresh(); return card;
