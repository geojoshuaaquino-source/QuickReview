...
const navGlyphs:Record<Tab,string>={Home:'⌂',Decks:'▤',Study:'◆',Library:'□',Settings:'⚙'};
...
function AppContent(){
 ...
 return <View style={s.safe}><StatusBar barStyle="dark-content" backgroundColor={T.colors.bg}/><View style={[s.app,{paddingTop:insets.top,paddingLeft:insets.left,paddingRight:insets.right}]}>{screen}</View><View style={[s.nav,{height:74+insets.bottom,paddingBottom:insets.bottom,paddingLeft:insets.left,paddingRight:insets.right}]}>{(['Home','Decks','Study','Library','Settings'] as Tab[]).map(x=>{const active=x===tab;return <Pressable key={x} onPress={()=>{setTab(x);if(x==='Study')setStudyMode('learn')}} accessibilityRole="tab" accessibilityState={{selected:active}} style={({pressed})=>[s.navItem,pressed&&s.pressed]}><View style={[s.navIcon,active&&s.navIconActive]}><Text style={[s.navGlyph,active&&s.navActive]}>{navGlyphs[x]}</Text></View><Text style={[s.navLabel,active&&s.navActive]}>{x}</Text></Pressable>})}</View></View>;
}
const s=StyleSheet.create({safe:{flex:1,backgroundColor:T.colors.bg},app:{flex:1},loading:{flex:1,alignItems:'center',justifyContent:'center',backgroundColor:T.colors.bg},loadingMark:{fontSize:42,fontWeight:'900',color:T.colors.accent},loadingText:{fontSize:12,fontWeight:'800',color:T.colors.muted,marginTop:8},nav:{borderTopWidth:1,borderTopColor:T.colors.line,backgroundColor:T.colors.surface,flexDirection:'row',paddingTop:9},navItem:{flex:1,alignItems:'center',justifyContent:'flex-start'},navIcon:{width:44,height:32,borderRadius:11,alignItems:'center',justifyContent:'center'},navIconActive:{backgroundColor:T.colors.accentSoft},navGlyph:{fontSize:17,color:T.colors.faint},navLabel:{fontSize:9,fontWeight:'800',color:T.colors.muted,marginTop:4},navActive:{color:T.colors.accent,fontWeight:'900'},pressed:{opacity:.62,transform:[{scale:.97}]}});