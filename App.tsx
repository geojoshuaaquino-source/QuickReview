...
export default function App(){
 const [tab,setTab]=useState<Tab>('Home'); const [selectedId,setSelectedId]=useState('biology');
 ...
 const selected=useMemo(()=>decks.find(d=>d.id===selectedId)||decks[0],[decks,selectedId]); const nav={go:setTab,select:setSelectedId};
 ...
 const screen=tab==='Home'?<HomeScreen decks={decks} reviews={reviews} schedulingEnabled={settings.schedulingEnabled} onStudy={id=>{setSelectedId(id);setTab('Study')}} onDecks={()=>setTab('Decks')} onLibrary={()=>setTab('Library')}:tab==='Decks'?<Decks decks={decks} setDecks={legacySetDecks} nav={nav}/>:tab==='Study'&&selected?<StudyScreen deck={selected} defaultSessionSize={settings.defaultSessionSize} onAdd={addCard} onStartSession={makeSession} onGrade={grade}/>:tab==='Library'?<LibraryScreen decks={decks} onToggleSuspend={toggleSuspend} onDelete={deleteCard} onUpdate={updateCard}/>:<SettingsScreen settings={settings} onChange={saveSettings}/>;
