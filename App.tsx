...
import { Home, Decks, Library, Settings } from './src/presentation/screens';
...
export default function App() {
  ...
  const { decks, loaded, createDeck, createCard, grade, makeSession, refresh } = useQuickReview(repo);
  ...
  const screen = tab === 'Home' ? <Home decks={decks} nav={nav} /> : tab === 'Decks' ? <Decks decks={decks} setDecks={legacySetDecks} nav={nav} /> : tab === 'Study' && selected ? <StudyScreen deck={selected} onAdd={addCard} onStartSession={makeSession} onGrade={grade} /> : tab === 'Library' ? <Library /> : <Settings />;
