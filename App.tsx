...
export default function App(){
 const [tab,setTab]=useState<Tab>('Home'),[decks,setDecks]=useState<Deck[]>([]),[selectedId,setSelectedId]=useState('biology'),[loaded,setLoaded]=useState(false);
 useEffect(()=>{repo.getDecks().then(x=>{setDecks(x);setLoaded(true)}).catch(()=>setLoaded(true))},[]);
 useEffect(()=>{if(loaded)decks.forEach(d=>{repo.saveDeck(d).catch(()=>{})})},[decks,loaded]);
 const selected=useMemo(()=>decks.find(d=>d.id===selectedId)||decks[0],[decks,selectedId]);
 const nav={go:setTab,select:setSelectedId};
 const addCard=async(card:Card)=>{await repo.saveCard(card);setDecks(await repo.getDecks())};
 ...
 const screen=tab==='Home'?<Home decks={decks} nav={nav}/>:tab==='Decks'?<Decks decks={decks} setDecks={setDecks} nav={nav}/>:tab==='Study'&&selected?<Study deck={selected} onAdd={addCard}/>:tab==='Library'?<Library/>:<Settings/>;
